...
# Copy private functions from dbplyr -------------------------------------------
filesToCopy <- c(
  "build-sql.R",
  "verb-copy-to.R",
  "sql-clause.R",
...
script <- c("# Autogenerated from PackageMaintenance.R. Do not change by hand")
for (i in seq_along(filesToCopy)) {
  file <- filesToCopy[i]
  lines <- readLines(file.path("..", "dbplyr", "R", file))
  lines <- lines[!grepl("^#'", lines)]
  lines <- lines[!grepl("assert_that", lines)]
  script <- c(script,
              "",
              sprintf("# From %s ----------------------------", file),
              lines)
}
writeLines(script, file.path("R", "DbplyrPrivateFunctions.R"))
